...
class ForecastabilityAdapter(nn.Module):
    ...
    def raw(self, x_norm):
        ...
        latent = torch.einsum("rt,bct->bcr", self.past_basis, x_ct)
        if self.channel_scale is not None:
            latent = latent * self.channel_scale.unsqueeze(0)
        pred = torch.einsum("hr,bcr->bch", self.future_basis, latent)
    ...
    def forward(self, x_norm, pred_base_norm):
        ...
        with torch.no_grad():
            a = alpha.detach().float().cpu()
            effective_rank = min(
                int(torch.linalg.matrix_rank(self.future_basis.detach().float()).cpu()),
                int(torch.linalg.matrix_rank(self.past_basis.detach().float()).cpu()),
            )
            self._last_diagnostics = {
