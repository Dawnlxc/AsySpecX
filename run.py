...
# AsySpecX
parser.add_argument('--spectral_lift', type=str, default='complex_mlp', choices=['complex_mlp', 'fits_linear'], help='AsySpecX spectral lift: old complex_mlp or FITS-style single complex linear')
parser.add_argument('--lift_sharing', type=str, default='shared', choices=['shared', 'individual', 'lowrank_channel'], help='AsySpecX spectral lift sharing mode')
parser.add_argument('--lift_rank', type=int, default=2, help='AsySpecX lowrank_channel lift rank')
parser.add_argument('--norm_mode', type=str, default='rin_noaffine', choices=['rin_noaffine', 'revin_affine', 'subtract_last', 'none'], help='AsySpecX normalization mode')
...
parser.add_argument('--gate_init_logit', type=float, default=None, help='AsySpecX gate logit init override for phase-1 experiments')
parser.add_argument('--gate_max', type=float, default=1.0, help='AsySpecX gate cap (1.0 = full strength, 0.2 = restrict)')
parser.add_argument('--gate_type', type=str, default='global', choices=['global', 'channel_band', 'hier_channel_band'], help='AsySpecX cross gate parameterization')
parser.add_argument('--residual_part', type=str, default=None, choices=['all', 'diag_only', 'offdiag_only', 'split'], help='AsySpecX residual decomposition; unset keeps mask_self_transfer compatibility')
parser.add_argument('--mask_self_transfer', type=int, default=0, help='AsySpecX safe cross: remove diagonal/self transfer')
parser.add_argument('--residual_clip_eta', type=float, default=-1.0, help='AsySpecX safe cross: cap residual RMS to eta * backbone RMS; <=0 disables')
parser.add_argument('--backcast_loss_weight', type=float, default=0.0, help='AsySpecX training: add weight * reconstruction loss over input lookback')
parser.add_argument('--force_cross_off', type=int, default=0, help='AsySpecX eval/debug: skip cross block even when configured')
parser.add_argument('--skip_dc_cross', type=int, default=1, help='AsySpecX safe cross: do not apply cross residual to DC bin')
parser.add_argument('--log_asyspecx_diagnostics', type=int, default=0, help='AsySpecX: print gate/residual diagnostics during training')
parser.add_argument('--eval_residual_part', type=str, default='default', choices=['default', 'none', 'all', 'diag_only', 'offdiag_only'], help='AsySpecX eval/debug: override residual part at forward time')
parser.add_argument('--gate_lr_mult', type=float, default=1.0, help='AsySpecX: learning-rate multiplier for gate parameters')
parser.add_argument('--self_gain_init_std', type=float, default=1e-3, help='AsySpecX self_band_gain complex gain init std')
parser.add_argument('--temporal_adapter', type=str, default='none', choices=['none', 'sparse_period', 'compact_period'], help='AsySpecX optional temporal adapter')
parser.add_argument('--period', type=int, default=24, help='AsySpecX sparse-period adapter period (single-period backward-compat)')
parser.add_argument('--periods', type=str, default='', help='AsySpecX multi-period list, e.g. "24,168" or "24+168" (+ avoids sbatch --export comma split); empty falls back to --period')
parser.add_argument('--periodic_init', type=str, default='seasonal_naive', choices=['seasonal_naive', 'zeros', 'small_random'], help='AsySpecX sparse-period adapter init')
parser.add_argument('--periodic_sharing', type=str, default='shared', choices=['shared', 'individual_channel'], help='AsySpecX sparse-period adapter weight sharing')
parser.add_argument('--temporal_fusion', type=str, default='convex', choices=['convex', 'additive'], help='AsySpecX temporal adapter fusion')
parser.add_argument('--temporal_gate_type', type=str, default='global', choices=['global', 'channel', 'horizon', 'horizon_channel'], help='AsySpecX temporal adapter gate type')
parser.add_argument('--temporal_gate_init_logit', type=float, default=-4.0, help='AsySpecX temporal adapter gate init logit')
parser.add_argument('--period_fusion', type=str, default='sum_gated', choices=['sum_gated', 'softmax'], help='AsySpecX multi-period fusion: per-period sigmoid gates or softmax over periods')
parser.add_argument('--period_gate_type', type=str, default='period', choices=['global', 'period', 'period_horizon', 'period_channel', 'period_horizon_channel'], help='AsySpecX multi-period gate parameterization')
parser.add_argument('--period_gate_init_logit', type=float, default=0.0, help='AsySpecX multi-period gate init logit (sum_gated: sigmoid(0)=0.5; softmax: uniform)')
parser.add_argument('--periodic_l1_weight', type=float, default=0.0, help='AsySpecX periodic-adapter L1 reg on in-mask weights (train-only)')
parser.add_argument('--periodic_l2_weight', type=float, default=0.0, help='AsySpecX periodic-adapter L2 reg on in-mask weights (train-only)')
parser.add_argument('--temporal_gate_l1_weight', type=float, default=0.0, help='AsySpecX Phase5: L1 on temporal fusion gate mean(sigmoid(alpha)); train-only, keeps periodic branch closed')
parser.add_argument('--val_num_segments', type=int, default=1, help='AsySpecX Phase5: split ordered validation into K contiguous segments and log val_mse_seg*/val_mae_seg* (1 = legacy behavior)')
# --- Phase 7: learned energy-controlled clip (cross residual) ---
parser.add_argument('--energy_control', type=str, default='none', choices=['none', 'learned_clip'], help='AsySpecX Phase7: none = static residual_clip_eta; learned_clip = learned per-component/band/channel eta (scale still capped at 1, never amplifies)')
parser.add_argument('--learned_clip_scope', type=str, default='component_channel_band', choices=['component_channel_band'], help='AsySpecX Phase7 learned_clip eta parameterization')
parser.add_argument('--learned_clip_eta_init', type=float, default=1.0, help='AsySpecX Phase7 learned_clip initial eta')
parser.add_argument('--learned_clip_eta_max', type=float, default=2.0, help='AsySpecX Phase7 learned_clip max eta (applied scale still clamped to 1)')
parser.add_argument('--clip_lr_mult', type=float, default=1.0, help='AsySpecX Phase7: learning-rate multiplier for learned_clip params')
# --- Phase 7: auto-period discovery (resolved at runner level; model still takes --periods) ---
parser.add_argument('--period_mode', type=str, default='manual', choices=['manual', 'auto_acf', 'auto_fft', 'union_auto'], help='AsySpecX Phase7/8 period source; auto_*/union_auto is resolved by scripts/discover_periods.py and passed as --periods')
parser.add_argument('--auto_period_topk', type=int, default=3, help='AsySpecX Phase7 auto-period top-k')
parser.add_argument('--auto_period_min', type=int, default=4, help='AsySpecX Phase7 auto-period min lag')
parser.add_argument('--auto_period_max', type=int, default=0, help='AsySpecX Phase7 auto-period max lag (0 = min(T,672))')
parser.add_argument('--auto_period_source', type=str, default='train', choices=['train', 'first_batch'], help='AsySpecX Phase7 auto-period data source (train only by default; never test)')
# --- Phase 7: patch-linear temporal residual ---
parser.add_argument('--patch_adapter', type=str, default='none', choices=['none', 'patch_linear'], help='AsySpecX Phase7 optional channel-independent patch-linear horizon residual')
# NOTE: --patch_len is already defined above (line ~43, type=int default=16); do not redefine (argparse conflict).
parser.add_argument('--patch_stride', type=int, default=8, help='AsySpecX Phase7 patch stride')
parser.add_argument('--patch_basis_dim', type=int, default=0, help='AsySpecX Phase7 patch basis dim (<=0 = mean+last feature)')
parser.add_argument('--patch_fusion', type=str, default='convex', choices=['convex', 'additive'], help='AsySpecX Phase7 patch fusion')
parser.add_argument('--patch_gate_type', type=str, default='horizon', choices=['global', 'channel', 'horizon', 'horizon_channel'], help='AsySpecX Phase7 patch gate type')
parser.add_argument('--patch_gate_init_logit', type=float, default=-6.0, help='AsySpecX Phase7 patch gate init logit (near-off)')
parser.add_argument('--patch_l1_weight', type=float, default=0.0, help='AsySpecX Phase7 patch projection L1 (train-only)')
parser.add_argument('--patch_l2_weight', type=float, default=0.0, help='AsySpecX Phase7 patch projection L2 (train-only)')
# --- Phase 7: prediction saving for offline ensemble ---
parser.add_argument('--save_predictions', type=int, default=0, help='AsySpecX Phase7: save val/test preds+targets as npz for offline ensemble')
parser.add_argument('--pred_save_dir', type=str, default='', help='AsySpecX Phase7: directory for saved predictions')
parser.add_argument('--pred_tag', type=str, default='', help='AsySpecX Phase7: prediction filename tag "arm__dataset__slL__plH__sdS" (default model_id)')
# --- Phase 8: period union + linear branch + Hydra branch fusion ---
parser.add_argument('--max_periods', type=int, default=5, help='AsySpecX Phase8 union_auto: cap on number of periods')
parser.add_argument('--linear_adapter', type=str, default='none', choices=['none', 'direct_linear', 'dlinear_decomp', 'multiscale_dlinear'], help='AsySpecX Phase8 optional linear/DLinear temporal branch')
parser.add_argument('--linear_sharing', type=str, default='shared', choices=['shared', 'individual'], help='AsySpecX Phase8 linear branch weight sharing')
parser.add_argument('--individual_linear_max_channels', type=int, default=64, help='AsySpecX Phase8: max channels for individual linear (else error)')
parser.add_argument('--linear_init', type=str, default='zeros', choices=['zeros', 'last', 'small_random'], help='AsySpecX Phase8 linear branch init')
parser.add_argument('--moving_avg_kernel', type=int, default=25, help='AsySpecX Phase8 dlinear_decomp moving-average kernel')
parser.add_argument('--multiscale_factors', type=str, default='1,2,4', help='AsySpecX Phase8 multiscale_dlinear downsample factors')
parser.add_argument('--multiscale_fusion', type=str, default='softmax', choices=['sum_gated', 'softmax'], help='AsySpecX Phase8 multiscale scale fusion')
parser.add_argument('--multiscale_gate_type', type=str, default='scale', choices=['global', 'scale', 'scale_horizon'], help='AsySpecX Phase8 multiscale gate type')
parser.add_argument('--linear_fusion', type=str, default='convex', choices=['convex', 'additive'], help='AsySpecX Phase8 linear branch fusion')
parser.add_argument('--linear_gate_type', type=str, default='horizon', choices=['global', 'channel', 'horizon', 'horizon_channel'], help='AsySpecX Phase8 linear fusion gate type')
parser.add_argument('--linear_gate_init_logit', type=float, default=-6.0, help='AsySpecX Phase8 linear gate init logit (near-off)')
parser.add_argument('--linear_l1_weight', type=float, default=0.0, help='AsySpecX Phase8 linear weight L1 (train-only)')
parser.add_argument('--linear_l2_weight', type=float, default=0.0, help='AsySpecX Phase8 linear weight L2 (train-only)')
parser.add_argument('--branch_fusion', type=str, default='sequential', choices=['sequential', 'softmax_static'], help='AsySpecX Phase8 Hydra: sequential (Phase7) or parallel softmax over branches')
parser.add_argument('--branch_fusion_scope', type=str, default='horizon', choices=['global', 'horizon', 'channel', 'horizon_channel'], help='AsySpecX Phase8 branch fusion scope')
parser.add_argument('--branch_init_main_logit', type=float, default=4.0, help='AsySpecX Phase8 branch fusion spec-branch init logit')
parser.add_argument('--branch_init_aux_logit', type=float, default=-4.0, help='AsySpecX Phase8 branch fusion aux-branch init logit')
# --- Phase 10: cycle-residual spectral forecasting ---
parser.add_argument('--cycle_residual', type=int, default=0, help='AsySpecX Phase10: subtract a learned phase template before spectral forecasting and add it back to the horizon')
parser.add_argument('--cycle_residual_rank', type=int, default=0, help='AsySpecX Phase10: factorized phase-template rank; <=0 uses the full cycle x channel table')
parser.add_argument('--cycle_residual_init_std', type=float, default=0.02, help='AsySpecX Phase10: initialization std for the nonzero factor in a rank-factorized cycle table')
# --- Phase 11: channel-separable past-to-future forecastability kernel ---
parser.add_argument('--forecast_kernel', type=str, default='none', choices=['none', 'lowrank_time'], help='AsySpecX Phase11: optional channel-separable low-rank past-to-future branch')
parser.add_argument('--forecast_kernel_rank', type=int, default=8, help='AsySpecX Phase11: temporal kernel rank')
parser.add_argument('--forecast_kernel_init', type=str, default='small_random', choices=['zeros', 'small_random', 'ridge_svd'], help='AsySpecX Phase11: factor initialization')
parser.add_argument('--forecast_kernel_init_path', type=str, default='', help='AsySpecX Phase11: train-only ridge/SVD artifact path')
parser.add_argument('--forecast_kernel_channel_scale', type=int, default=0, help='AsySpecX Phase11: learn a per-channel, per-mode amplitude; never mixes channels')
# --- Phase 11 Stage B: real spectral-mixture envelope (complex phase locked) ---
parser.add_argument('--forecast_kernel_spectral_mixtures', type=int, default=0, help='Stage B: number of Gaussian components in the positive real frequency envelope; 0 keeps exact Stage A')
parser.add_argument('--forecast_kernel_sm_sharing', type=str, default='mode', choices=['shared', 'mode'], help='Stage B: share one envelope or learn one per low-rank temporal mode')
parser.add_argument('--forecast_kernel_sm_max_log_gain', type=float, default=1.0, help='Stage B: bounded maximum log-amplitude modulation')
parser.add_argument('--forecast_kernel_sm_min_width', type=float, default=0.02, help='Stage B: minimum normalized Gaussian spectral width')
parser.add_argument('--forecast_kernel_sm_init_width', type=float, default=0.20, help='Stage B: initial normalized Gaussian spectral width')
parser.add_argument('--forecast_kernel_sm_center_max_shift', type=float, default=0.15, help='Stage B: maximum learned shift from fixed spectral center anchors')
parser.add_argument('--forecast_kernel_sm_base_trainable', type=int, default=1, choices=[0, 1], help='Stage B: keep the underlying time basis trainable; 0 is the frozen-basis compression control')
parser.add_argument('--forecast_kernel_extension_shrink', type=str, default='none', choices=['none', 'tail2_linear'], help='Stage D: deterministic horizon-safe shrinkage; tail2_linear is exact Stage A for H <= 2L')
parser.add_argument('--forecast_kernel_phase_basis_dim', type=int, default=0, help='Stage B.2: number of smooth cosine horizon bases for bounded within-variable complex phase; 0 keeps the real-SM model')
parser.add_argument('--forecast_kernel_phase_max', type=float, default=0.7853981633974483, help='Stage B.2: maximum absolute horizon phase in radians')
parser.add_argument('--forecast_kernel_fusion', type=str, default='convex', choices=['convex', 'additive'], help='AsySpecX Phase11: bounded fusion with the existing forecast')
parser.add_argument('--forecast_kernel_gate_type', type=str, default='horizon', choices=['global', 'channel', 'horizon', 'horizon_channel'], help='AsySpecX Phase11: forecast branch gate parameterization')
parser.add_argument('--forecast_kernel_gate_init_logit', type=float, default=-6.0, help='AsySpecX Phase11: near-off fusion gate initialization')
parser.add_argument('--forecast_kernel_l1_weight', type=float, default=0.0, help='AsySpecX Phase11: train-only L1 on temporal factors')
parser.add_argument('--forecast_kernel_l2_weight', type=float, default=0.0, help='AsySpecX Phase11: train-only L2 on temporal factors')
# --- Phase 9: SafeRoute orchestration (model forward remains unchanged) ---
parser.add_argument('--expert_manifest', type=str, default='', help='Phase9 frozen-expert manifest path')
parser.add_argument('--anchor_expert', type=str, default='anchor', help='Phase9 mandatory fallback expert')
parser.add_argument('--expert_seeds', type=str, default='2024,2025,2026', help='Phase9 comma-separated frozen expert seeds')
parser.add_argument('--router_num_horizon_blocks', type=int, default=4, help='Phase9 contiguous horizon blocks')
parser.add_argument('--router_target', type=str, default='advantage', choices=['advantage', 'log_relative_regret'], help='Phase9 router target')
parser.add_argument('--router_backend', type=str, default='xgboost', choices=['xgboost', 'hist_gradient_boosting', 'logistic_best_expert'], help='Phase9 compact router backend')
parser.add_argument('--router_scope', type=str, default='cell', choices=['cell', 'dataset', 'family', 'global'], help='Phase9 router sharing scope')
parser.add_argument('--router_min_samples', type=int, default=256, help='Phase9 minimum labelled rows per expert/block router')
parser.add_argument('--router_cv_folds', type=int, default=4, help='Phase9 purged blocked CV folds')
parser.add_argument('--router_purge_steps', type=int, default=0, help='Phase9 purge gap; 0 uses pred_len')
parser.add_argument('--router_meta_source', type=str, default='val', choices=['val', 'rolling_oof'], help='Phase9 router metadata protocol')
parser.add_argument('--router_oof_seed', type=int, default=2024, help='Phase9 rolling-OOF expert seed')
parser.add_argument('--router_confidence_alpha', type=float, default=0.1, help='Phase9 LCB miscoverage alpha')
parser.add_argument('--router_min_gain', type=float, default=0.0, help='Phase9 minimum safe advantage needed to activate')
parser.add_argument('--router_decision', type=str, default='safe_top1_blend', choices=['hard_top1', 'safe_top1_blend', 'safe_multi_mix'], help='Phase9 bounded routing decision')
parser.add_argument('--router_full_gain', type=float, default=0.02, help='Phase9 safe score giving blend alpha=1')
parser.add_argument('--router_uncertainty_beta', type=float, default=0.1, help='Phase9 seed-uncertainty penalty')
parser.add_argument('--router_temperature', type=float, default=0.1, help='Phase9 safe multi-mix temperature')
parser.add_argument('--router_channel_groups', type=int, default=1, help='Phase9 optional channel groups; keep 1 until audit gate passes')
parser.add_argument('--save_full_predictions', type=int, default=0, choices=[0, 1], help='Phase9 opt-in chunked memmap export; default stores compact metadata only')
...
# FreqSelectiveCross
parser.add_argument('--cross_mode', type=str, default='hybrid', choices=['none', 'herm', 'asym', 'hybrid', 'asym_lowrank', 'self_band_gain'], help='Cross branch mode; AsySpecX uses none/asym_lowrank/self_band_gain')
...
parser.add_argument('--eval_test_during_train', type=int, default=1, help='1 keeps legacy epoch-wise test logging; Phase11 screening sets 0 to avoid test peeking')
parser.add_argument('--defer_test', type=int, default=0, help='1 stops after validation-selected training; evaluate the frozen checkpoint in a later command')
...
if args.is_training:
    for ii in range(args.itr):
        ...
        if not args.defer_test:
            ...
            print('>>>>>>>testing deferred (validation-only screen) : {}<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
