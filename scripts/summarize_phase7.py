...
def main():
    ...
    p.add_argument("--anchor_arm", default="phase6_asx_period_multi")
    p.add_argument("--output_dir", default="phase7_results/merged")
    args = p.parse_args()
    ...
    L = ["# Phase 7-Breakthrough Summary", ""]
    ...
    # which NEW arms beat OLD per-cell best
    L += ["## New (phase7_*) Arms Improving Old (phase6_*) Best Cells", "",
    ...
    for key in sorted(by_cell):
        ...
        old = {a: v for a, v in am.items() if a.startswith("phase6_")}
        new = {a: v for a, v in am.items() if a.startswith("phase7_")}
    ...
    L += ["", f"- cells where a phase7 arm beats the phase6 best: {n_improved}", ""]
