#!/bin/bash
# AsySpecX benchmark — FreTS on electricity.
# Hyperparams: aikunyi/FreTS run_longExp.py defaults (no per-dataset scripts).
set -euo pipefail
cd "$(cd -- "$(dirname -- "$0")/../.." && pwd)"
...
if [ -n "${SL_OVERRIDE:-}" ]; then
  SEQ_LENS=($SL_OVERRIDE)
  PRED_LENS_OVERRIDE="${PL_OVERRIDE:-96}"
elif [ "${SMOKE:-0}" = "1" ]; then
  SEQ_LENS=(96 720)
...
for sl in "${SEQ_LENS[@]}"; do
  ...
  for pl in $PL_LIST; do
    ...
    echo "[$(date '+%F %T')] $MODEL $data_key sl=$sl pl=$pl"
    python -u run.py \
      ...
      --channel_independence 0 --dropout $dropout \
