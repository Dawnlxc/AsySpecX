#!/bin/bash
# Phase 8-Hydra selection over merged Phase6+7+8 pool. 4 validation-only variants.
set -euo pipefail
cd "$(cd -- "$(dirname -- "$0")/.." && pwd)"

ROOT="${ROOT:-phase8_results/merged}"
CSV="${CSV:-${ROOT}/results.csv}"
PYTHON="${PYTHON:-python}"
UNRESTRICTED="${UNRESTRICTED_JSON:-configs/selection/phase8_unrestricted.json}"
POLICY="${POLICY_JSON:-configs/selection/phase6_policy_family.json}"
PREFER="phase6_asx_individual_revin,phase6_asx_individual,phase8_auto_acf_patchlinear_split_clip05,phase7_period_multi_auto_acf_patchlinear,phase8_auto_acf_patchlinear,phase6_asx_cross_clip05,phase6_asx_period_multi,phase6_asx_individual_period"
mkdir -p "$ROOT"
...
if "$PYTHON" scripts/select_by_validation.py --csv "$CSV" \
    ...
    echo "segment_robust ok"; else echo "segment_robust skipped (no val_mse_seg*)"; fi
...
echo "Phase 8 selection done. unrestricted_mean is the main result."
