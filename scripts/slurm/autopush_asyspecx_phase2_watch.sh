#!/bin/bash
# Wait for AsySpecX Phase 2 Slurm jobs, aggregate, push, email.
set -euo pipefail

cd "$(cd -- "$(dirname -- "$0")/../.." && pwd)"

OUTROOT="${1:-phase2_results/main}"
MAIL_TO="${2:-yind7@outlook.com}"
NO_PUSH="${3:-0}"
POLL_SECONDS="${POLL_SECONDS:-300}"
LOG_FILE="logs/autopush_asyspecx_phase2.log"
...
if command -v mail >/dev/null 2>&1; then
    {
        ...
        echo
        sed -n '1,140p' "$OUTROOT/summary.md" 2>/dev/null || true
    } | mail -s "[AsySpecX phase2] done" "$MAIL_TO" && mail_status="ok" || mail_status="failed"
