...
RUN_TAG="${RUN_TAG:-phase11_stagec_wave1_0716v1}"
OUTROOT="${OUTROOT:-phase11_results/stagec_wave1}"
MANIFEST="${MANIFEST:-configs/phase11_stagec_wave1_screen.tsv}"
EPOCHS="${EPOCHS:-30}"
...
printf 'job_id\tbase_profile\tarm\tdataset\tseq_len\tpred_len\tseed\tcut_freq\n' >"$submitted_manifest"

submitted=0
while IFS=$'\t' read -r base_profile arm dataset seq_len pred_len seed cut_freq; do
    [ -z "$base_profile" ] && continue
    [[ "$base_profile" == \#* ]] && continue
    job_name="a11c_${arm}_${dataset}_s${seq_len}_p${pred_len}_d${seed}"
    ...
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        "$job_id" "$base_profile" "$arm" "$dataset" "$seq_len" \
        "$pred_len" "$seed" "$cut_freq" >>"$submitted_manifest"
    submitted=$((submitted + 1))
done <"$MANIFEST"

echo "[phase11-stagec-submit] submitted=$submitted dry_run=$DRY_RUN manifest=$submitted_manifest"
