#!/bin/bash
# Submit AsySpecX Phase 7-Breakthrough jobs to Slurm.
# 8 arms (COMPACT=1 -> 5). Auto-period arms resolve PERIODS in-job (train only).
# PERIODS comma->plus for --export. PEMS seq_len=96 only unless RUN_PEMS_SEQ720=1.
set -euo pipefail
...
OUTROOT="${OUTROOT:-phase7_results/breakthrough}"
...
if [ "$COMPACT" = "1" ]; then DEFAULT_ARMS="$PHASE7_ARMS_COMPACT"; else DEFAULT_ARMS="$PHASE7_ARMS_FULL"; fi
...
submitted=0
echo "AsySpecX phase7 breakthrough submit (COMPACT=$COMPACT)"
echo "ARMS=$ARMS"
...
for arm in $ARMS; do
    ...
    for dataset in $DATASETS; do
        ...
        for seed in $SEEDS; do
            for sl in $sls; do
                for pl in $pls; do
                    jobname="asx7_${arm}_${dataset}_s${sl}_p${pl}_d${seed}"
                    ...
                    cmd=(sbatch -J "$jobname" --account="$ACCOUNT" --partition="$PARTITION" --time="$TIME_LIMIT" --export="$export_args" scripts/slurm/asyspecx_phase7_run.sbatch)
...
if [ "$NO_WATCH" != "1" ]; then
    mkdir -p logs
    echo "Starting watcher: logs/autopush_asyspecx_phase7.log"
    BASELINE_CSV="$BASELINE_CSV" MAIL_SUBJECT="$MAIL_SUBJECT" PHASE6_CSV="$PHASE6_CSV" SAVE_PREDICTIONS="$SAVE_PREDICTIONS" \
    nohup bash scripts/slurm/autopush_asyspecx_phase7_watch.sh "$OUTROOT" "$MAIL_TO" "$NO_PUSH" \
        > logs/autopush_asyspecx_phase7.nohup 2>&1 &
fi
