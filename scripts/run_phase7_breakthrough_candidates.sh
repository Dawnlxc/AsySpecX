#!/bin/bash
# Phase 7-Breakthrough candidate runner (local matrix).
#
# 8 arms (COMPACT=1 -> 5). Auto-period arms resolve PERIODS via
...
OUT_ROOT="${OUT_ROOT:-phase7_results/breakthrough}"
...
if [ "$COMPACT" = "1" ]; then ARMS="$PHASE7_ARMS_COMPACT"; else ARMS="$PHASE7_ARMS_FULL"; fi
NARMS=$(printf '%s\n' $ARMS | wc -w)
mkdir -p logs/AsySpecX_phase7 checkpoints results "$OUT_ROOT" "$OUT_ROOT/auto_periods"
...
echo "Phase 7 estimate: $count runs (arms=$NARMS [COMPACT=$COMPACT], datasets=[$DATASETS], seq_lens=[$SEQ_LENS], seeds=[$SEEDS], RUN_PEMS_SEQ720=$RUN_PEMS_SEQ720)"
echo "OUT_ROOT=$OUT_ROOT SAVE_PREDICTIONS=$SAVE_PREDICTIONS VAL_NUM_SEGMENTS=$VAL_NUM_SEGMENTS DRY_RUN=$DRY_RUN"

resolve_periods() {  # resolve_periods <dataset> <sl> <manual_periods> <auto_method>
    local dataset="$1" sl="$2" manual="$3" method="$4"
    if [ -z "$method" ]; then echo "$manual"; return; fi
    local cache="$OUT_ROOT/auto_periods/${dataset}_sl${sl}_${method}.json"
    local out
    if [ -f "$cache" ]; then
        ...
        out=$("$PYTHON" scripts/discover_periods.py --dataset "$dataset" --data "$data_name" \
            ...
            --fallback_periods "$manual" --output "$cache" 2>>logs/AsySpecX_phase7/discover.log || echo "$manual")
    fi
    [ -z "$out" ] && out="$manual"
    echo "$out"
}
...
for dataset in $DATASETS; do
    ...
    for sl in $sls; do
        ...
        for arm in $ARMS; do
            ...
            method="$(phase7_arm_is_auto "$arm")"
            periods="$(resolve_periods "$dataset" "$sl" "$manual_periods" "$method")"
            first_period="${periods%%,*}"; first_period="${first_period%%+*}"
            mapfile -t FLAGS < <(phase7_arm_flags "$arm" "$periods" "$first_period")
            for seed in $SEEDS; do
                for pl in $pred_lens; do
                    model_id="AsySpecX_${arm}_${data_key}_L${sl}_H${pl}"
                    log_file="logs/AsySpecX_phase7/${model_id}_sd${seed}.log"
                    pred_tag="${arm}__${data_key}__sl${sl}__pl${pl}__sd${seed}"
                    cmd=(python -u run.py
                    ...
                    if [ "$SAVE_PREDICTIONS" = "1" ]; then
                        cmd+=(--save_predictions 1 --pred_save_dir "$OUT_ROOT/predictions" --pred_tag "$pred_tag")
                    fi
                    if [ "$DRY_RUN" = "1" ]; then
                        printf '[dry]'; printf ' %q' "${cmd[@]}"; printf '\n'
                    else
                        echo "[$(date '+%F %T')] $arm $data_key L=$sl H=$pl sd=$seed periods=$periods method=${method:-manual}"
                        "${cmd[@]}" > "$log_file" 2>&1
                    fi
                done
            done
...
echo "Phase 7 candidates done (dry_run=$DRY_RUN)."
